...
import { getRankFromRating, SAFE_PROFILE_COLUMNS } from "@/lib/types";
...
export default function DashboardPage() {
  ...
  const { data: recentMatches } = useQuery({
    ...
    enabled: !!profile,
    queryFn: async () => {
      const { data } = await supabase.from("matches").select("*").or(`player1_id.eq.${profile!.id},player2_id.eq.${profile!.id}`).order("created_at", { ascending: false }).limit(5);
  ...
  const { data: topPlayers } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("profiles").select(SAFE_PROFILE_COLUMNS).order("rating", { ascending: false }).limit(5);
      ...
      <div className="mb-8 rounded-2xl border border-border arena-card p-6">
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          <div className="flex items-center gap-4">
            <img src={profile.avatar || ""} alt="avatar" className="h-16 w-16 rounded-2xl" />
            <div>
              ...
              <div className="mt-1 flex items-center gap-3">
                ...
                {profile.cf_handle && (
                  <span className="text-sm text-muted-foreground font-mono">CF: {profile.cf_handle} ({profile.cf_rating || "?"})</span>
                )}
              </div>
            </div>
          </div>
          <Link to="/matchmaking" className="inline-flex items-center gap-2 rounded-xl bg-primary px-6 py-3 font-display font-semibold text-primary-foreground transition-all hover:glow-green">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <div className="rounded-xl border border-border bg-card p-6">
          <div className="mb-4 flex items-center justify-between">
            ...
            <Link to="/matchmaking" className="text-sm text-primary hover:underline">View all</Link>
          </div>
                ...
                return (
                  <Link key={match.id} to={`/match/${match.id}`} className="flex items-center justify-between rounded-lg border border-border bg-secondary/50 p-3 transition-colors hover:border-primary/30">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm font-mono">{match.contest_id}{match.problem_index}</span>
                      <span className={`text-xs ${match.status === "active" ? "text-neon-cyan" : "text-muted-foreground"}`}>{match.status}</span>
                    </div>
          ...
          <div className="space-y-2">
            {topPlayers?.map((player, i) => (
              <div key={player.id} className="flex items-center gap-3">
                ...
                <div className="flex-1"><PlayerCard profile={player} compact /></div>
              </div>
            ))}
          </div>
        </div>
